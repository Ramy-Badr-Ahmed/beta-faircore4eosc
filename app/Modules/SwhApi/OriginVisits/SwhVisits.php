...
class SwhVisits extends SyncHTTP
{
    ...
    public function buildTreeNodes(): iterable|Throwable
    {
        ...
        array_walk($allSnapshots, function ($snapshot) use(&$snapshots, $revisions){
            ...
            $snapshotEdges = TreeEdges::getSnapshotEdges($snapshot);
            ...
            array_walk($snapshotEdges, function ($revisionID, $branch) use(&$revisions){
                $revisionEdges = TreeEdges::getRevisionEdges($revisionID);

                $directoryEdges = TreeEdges::getDirectoryEdges($revisionEdges["directory"]);
                ...
                $directoryEdges = Arr::map($directoryEdges, function ($directoryNode){
                    try {
                        ...
                        return [ $dirID => TreeEdges::getDirectoryEdges($directoryNode)];
        ...
        $stop = microtime(true);
        self::addLogs("--> Tree built in: ".round($stop - $start, 2)." seconds");
