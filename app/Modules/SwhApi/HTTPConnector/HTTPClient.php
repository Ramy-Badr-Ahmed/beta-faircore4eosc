...
abstract class HTTPClient
{
                    ...
                    if(isset($e) && !$e instanceof GuzzleRequestException) {
                        switch (true) {
                            ...
                            case $e->response->status() === 403:
                                ...
                                if(self::$apiURL === config('swh.staging.api-url')) {
                                    self::addLogs($retryMessage . " : Yes. Reason --> {$e->getMessage()}");

                                    $request->withToken(config('swh.staging.token'));
                                    return true;
                                }
                                break;
     ...
     * @param string $endPoint
     * @param Collection $append2Url
     * @param callable|NULL $patternCallback
     * @return bool
     * @throws Exception
     */
    ...
    private static function isExpectedPattern(string $endPoint, Collection $append2Url) : bool
    {
        ...
        $validator = Validator::make($append2Url->toArray(),
            [
                0 => ['url', 'max:255', 'regex: /^[a-f0-9]{40}(?:\/\??\S*)?$/i']
            ],
            [
