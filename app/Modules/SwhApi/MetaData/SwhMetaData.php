...
abstract class SwhMetaData
{
    ...
    public static function getOriginMetaData(string $url): array|Throwable
    {
        ...
        $sync = new SyncHTTP();
        $forgeList = $sync->invokeHTTP('GET', $fullOriginData['metadata_authorities_url'])->json();

        $metadataListUrls = Arr::pluck($forgeList, 'metadata_list_url');

        $swhMeta = [];
        foreach ($metadataListUrls as $key => $url){
            $swhMeta["metadata_list_url_".$key+1] = $sync->invokeHTTP('GET', $url)->json();

            foreach ($swhMeta["metadata_list_url_".$key+1] as $idx => $arr){
                $swhMeta["metadata_list_url_".$key+1][$idx]['metadata_url'] = $sync->invokeHTTP('GET', $arr['metadata_url'])->json();
            }
        }
        return $swhMeta;
    }
