...
class ReleaseMaintenanceMode extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:release-maintenance-mode';
     ...
     * @var string
     */
    protected $description = 'Reset Maintenance Parameters';
    ...
    {
        $configContents = preg_replace("/'enforce_https'\s*=>\s*(.+)/", "'enforce_https' => true,", $this->configContents);
        File::put($this->configPath, $configContents);

        $envContents = preg_replace_array("/APP_DEBUG\s*=\s*(.+)|#APP_URL|#ASSET_URL/", ["APP_DEBUG=false", "APP_URL", "ASSET_URL"], $this->envContents);
